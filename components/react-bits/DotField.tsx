...
const DotField = memo(({
  ...
  gradientTo = 'rgba(194, 65, 12, 0.10)',
  glowColor = 'rgba(252, 217, 168, 0.25)',
  className,
  style,
  ...rest
}: DotFieldProps) => {
  ...
  const reactId = useId();
  const glowId = `dot-field-glow-${reactId.replace(/[^a-zA-Z0-9-_]/g, '')}`;
  ...
  return (
    <div className={cn("w-full h-full relative", className)} style={style} {...rest}>
      <canvas
        ref={canvasRef}
        style={{
          position: 'absolute',
